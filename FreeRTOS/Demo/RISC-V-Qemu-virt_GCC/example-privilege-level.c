...
extern void vSendStringRaw( const char *s );

#if 1
#define PR_INFO(...) vSendStringRaw(__VA_ARGS__)
#else
#define PR_INFO(...)
#endif
...
void supervisor_mode_trap_handler(uintptr_t epc, int cause)
{
	int inst_len = 4;
	uintptr_t value;
    char temp[32];
	...
	if (cause == METAL_ECALL_S_EXCEPTION_CODE) {
        ...
        __asm__ volatile("csrw mepc, %0" ::"r"(epc + inst_len));
	} else {
		exit(7);
	}
}
...
*/
void user_mode_trap_handler(uintptr_t epc, int cause)
{
	int inst_len = 4;
	uintptr_t value;
    char temp[32];
...
{
    vSendStringRaw("In supervisor mode\n");
...
int test_privilege_main(void)
{
	...
	if (!cpu) {
		vSendStringRaw("Unable to get CPU handle\n");
		return 1;
	}
    vSendStringRaw("get CPU handle\n");

#if 0
	cpu_intr = metal_cpu_interrupt_controller(cpu);
	if (!cpu_intr) {
		vSendStringRaw("Unable to get CPU Interrupt handle\n");
		return 2;
	}
    vSendStringRaw("get CPU Interrupt handle\n");

	metal_interrupt_init(cpu_intr);
    vSendStringRaw("metal_interrupt_init\n");
	...
	if (ret < 0) {
		vSendStringRaw("Failed to register S-Mode exception handler\n");
		return 3;
	}
    vSendStringRaw("register S-Mode exception handler\n");

	ret = metal_cpu_exception_register(cpu,
					   METAL_ECALL_U_EXCEPTION_CODE,
					   user_mode_trap_handler);
	if (ret < 0) {
		vSendStringRaw("Failed to register U-Mode exception handler\n");
		return 4;
	}
#endif
	...
	if (!pmp) {
		vSendStringRaw("Unable to get PMP Device\n");
		return 5;
	}
	metal_pmp_init(pmp);
